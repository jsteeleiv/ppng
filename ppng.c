...
void read_bytes_(FILE *file, void *buf, size_t buf_cap, const char *src_file,
                 int src_line) {
  ...
  if (n != 1) {
    if (ferror(file)) {
      ...
      fprintf(stderr, "ERROR: could not read %zu  bytes from file: (EOF)\n",
              buf_cap);
      exit(1);
    } else {
      quit = true; // band-aid?
...
void write_bytes_(FILE *file, void *buf, size_t buf_cap, const char *src_file,
                  int src_line) {
...
void flip_bytes(void *buf_,
                size_t buf_cap) { // needed for *big endian* byte ordering
  uint8_t *buf = buf_;
...
*buf, size_t buf_cap,
                                                                        //uint8_t
chunk_type[4]), uint32_t chunk_crc)
{
